...
def patch_editable_wheel_command():
    ...
    distribution's egg-info, and the installed dist-info directory created by the editable_wheel command. This is so
    we can resolve entry points re-generated by ``python -m plux plugins`` from the source code, while using editable
    installs via ``pip install -e .`` into the venv.
    ...
    The .egg-info directory will correctly contain an entry-points.txt, but not the generated dist-info directory in
    site-packages. This is a problem under certain circumstances because importlib may not find the entry points. The
    reasons why dist-info doesn't contain the entry_points.txt are because of how pip works, but would ultimately
    also not help, as we would have to ``pip install -e .`` again every time the plugins change. So a dynamic
    approach is better, which the linking facilitates. During runtime, simply follow the link left behind in
    ``entry_points_editable.txt`` to the real ``entry_points.txt`` in the .egg-info directory.

    TODO: this is a hacky patch that relies on setuptools internals. It would be better to find a clean way to
      completely overwrite the ``editable_wheel`` command, perhaps via the command class resolution mechanism. i looked
      into that for an hour or so and concluded that this is an acceptable workaround for now.
    """
